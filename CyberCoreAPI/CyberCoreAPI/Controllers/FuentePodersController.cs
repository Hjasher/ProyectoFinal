...
{
    public class FuentePodersController : ApiController
    {
        ...
        public IHttpActionResult GetFuentePoder()
        {
            var fp = db.Componentes.OfType<FuentePoder>().ToList();

            if (fp == null)
            {
                return Content(HttpStatusCode.NotFound, "No se encontraron fuentes de poder en la base de datos");
            }
            return Ok(fp);
        }
        ...
        [HttpGet]
        [ResponseType(typeof(FuentePoder))]

        public IHttpActionResult GetFuentePoder(int id)
        {
            var fp = db.Componentes.OfType<FuentePoder>().FirstOrDefault(i => i.ID == id);
            if (fp == null)
            {
                return Content(HttpStatusCode.NotFound, $"No se encontró la fuente de poder con el id: {id}");
            }
        ...
        [HttpPut]
        public IHttpActionResult PutFuentePoder(int id, FuentePoder fp)
        {
            ...
            var fpExistente = db.Componentes.OfType<FuentePoder>().FirstOrDefault(p => p.ID == id);
            if (fpExistente == null)
            {
                return Content(HttpStatusCode.NotFound, $"No se encontró la fuente de poder con el id: {id}");
            }

            fpExistente.Nombre = fp.Nombre;
            fpExistente.Marca = fp.Marca;
            fpExistente.Precio = fp.Precio;
            fpExistente.Descripcion = fp.Descripcion;
            fpExistente.Stock = fp.Stock;
            fpExistente.Potencia = fp.Potencia;
            fpExistente.Certificacion = fp.Certificacion;
            ...
            {
                Mensaje = "La fuente de poder se actualizó correctamente",
                Datos_Nuevos = fpExistente
        ...
        [ResponseType(typeof(FuentePoder))]
        [HttpPost]
        public IHttpActionResult PostFuentePoder(FuentePoder fuentePoder)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            db.Componentes.Add(fuentePoder);
            db.SaveChanges();

            return CreatedAtRoute("DefaultApi", new { id = fuentePoder.ID }, fuentePoder);
        }
        ...
        [ResponseType(typeof(FuentePoder))]
        [HttpDelete]
        public IHttpActionResult DeleteFuentePoder(int id)
        {
            var fuentePoder = db.Componentes.OfType<FuentePoder>().FirstOrDefault(p => p.ID == id);
            if (fuentePoder == null)
            {
                return Content(HttpStatusCode.NotFound, $"No se encontró la fuente de poder con el id: {id}");
            }

            db.Componentes.Remove(fuentePoder);
            db.SaveChanges();

            return Ok(fuentePoder);
        }
        ...
        private bool FuentePoderExists(int id)
        {
